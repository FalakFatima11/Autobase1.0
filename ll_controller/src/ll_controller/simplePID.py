...
class SimplePID(object):
    ...
    def __init__(
        ...
        error_map=None,
        error_mag_integral = None # changed
    ):
        """
        ...
        self.error_mag_integral = error_mag_integral # Turn on integral only if error is within this value
    ...
    def __call__(self, input_, dt=None):
        ...
        if self.sample_time is not None and dt < self.sample_time and self._last_output is not None:
            # Only up            print(self._proportional)date every sample_time seconds
            return self._last_output

        # Compute error terms
        error = self.setpoint - input_
        d_input = input_ - (self._last_input if (self._last_input is not None) else input_)
        # print('Input, Last Input, DT')
        # print(input, self._last_input, dt)
        ...
        # Compute final output (to avoid integral windup)
        if self.error_mag_integral is not None and abs(error) < self.error_mag_integral:
